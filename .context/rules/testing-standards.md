<metadata updated="2025-11-05" version="3.0.0" name="测试规范（可选逻辑层测试）">
  <keywords>单元测试,纯函数,可选测试,简化测试,Node环境</keywords>
</metadata>

# 🧪 测试规范（可选逻辑层测试）

> 本规范采用灵活的测试策略：**不强制要求任何测试**，如需测试则仅针对**同步、纯函数、无副作用**的逻辑。严禁测试组件渲染、用户交互、网络请求或异步流程。
...
### 1. 测试文件要求（可选）
- **测试不是强制的**：项目可以选择不编写任何测试
- 如需测试，文件后缀：**必须为 `.test.ts`**（禁止 `.tsx`）
...
> **测试是可选的，不是强制的**。项目可以根据需要选择是否编写测试。
>
> 如需测试，**测试的是"函数的返回值"，不是"组件的表现"。**
>
